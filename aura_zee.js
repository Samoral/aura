...
//localStorage.clear()
 var storage = JSON.parse(localStorage.getItem("storage")) || {sublec:[],sublec_jsonM:[],note_scroll:[],notepad:[],numberCounter:0};
  ...
  $(".seeknal_navBar_list p").click(function() {
    if($(this).attr("allow")!=="yes") {
      alert("This feature is not yet available in your region. Check back later")
    }
  })
...
function uI() {
   $(".dark_deep").css({"background":"#120822","color":"white"});
      ...
      $(".dark_list_or_important").css("background","#1b1636");
      $(".lessons").css("background","#16122a")
...
var processedContinue_click_func = function() {
  ...
  if(value >= signalNote[0].scrollWidth - mainPage_width) {
    ...
    $(".lessons,.dark_list_or_important").css("background","#1b1636")
      $(".dark_transparent .data_x2d2, .dark_transparent .title").css("color","#a8dcf6");
      $(".dark_progress").css("background","#a855f7"); 
  }
...
let numberCounting = storage.numberCounter;
    ...
    let sublec_json = ["More On Signals", "Classes Of Signals","Odd Signals","Other Classes Of Signals","Operations On Signals","Signal Addition And Multiplication"];
...
storage.note_scroll[index]=value;
  ...
  if(value >= $(".signalNote").eq(index)[0].scrollWidth - mainPage_width) {
    ...
    $(".contin").parent().parent().siblings(".mainPage").children(".content").children(".lecture_wrapper").children(".subLec").eq(index+1).html(`<div class="lessons dark_transparent" id="lesson_${index+1}">
        ...
        <div style="width: 100%;">
        <small style="color: grey;font-size: 11px; font-weight: bolder;font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif;">LESSON ${index+2} <i class="fa fa-check-circle finished_course" style="color:green;display:none"></i></small>
        <span class="page_counter" style="opacity:0;">${index+1}</span>
...
$(document).on("click",".prev_button,.back_button",back_click_func)
